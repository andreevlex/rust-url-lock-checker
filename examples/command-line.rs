...
use urllockcheck::{DetailInfo, UrlLockChecker};
...
fn main() {
    let app_m = App::new("url-lock-checker")
        ...
        .about(
            "Консольная программа для проверки сайта на наличие в запрещенном списке
             через API сайта https://antizapret.info/",
        )
        .arg(
            Arg::with_name("check")
                .short("c")
                .long("check")
                .help(
                    "проверить сайт по доменному имени или ip адресу",
                )
                .takes_value(true)
                .value_name("DOMAIN_NAME"),
        )
        .arg(
            Arg::with_name("print-ip")
                .short("p")
                .long("print-ip")
                .help(
                    "получить список заблокированных ip адресов по доменному имени",
                )
                .takes_value(true)
                .value_name("DOMAIN_NAME"),
        )
        .arg(
            Arg::with_name("update-date")
                .short("d")
                .long("update-date")
                .help(
                    "показать дату обновления данных по доменному имени",
                )
                .takes_value(true)
                .value_name("DOMAIN_NAME"),
        )
        .arg(
            Arg::with_name("print-url")
                .short("u")
                .long("print-url")
                .help(
                    "получить список заблокированных url адресов по доменному имени",
                )
                .takes_value(true)
                .value_name("DOMAIN_NAME"),
        )
